...
# Overview  (نظرة عامة)
A collection of photographs showing found  _Béton_ have been stored in a dedicated image dataset. With this dataset, I am exploring ways that _‘Machine Learners’_ refers to humans as much as it does to computers. I found it interesting to bring focus on an unrecognised habit within Lebanese society, which is the free exchange of _Béton_, or concrete block, vernacularly passed around whenever spotted/needed. As a single _‘variable’_ which represents experimentation and fragmentation, _Béton_ can be computationally seen within the ‘Latent Space.’ In this hidden layer of machine learning, input data is broken down and apart, and tries to re-assemble itself by learning possible compositions. While the results are ambiguous, unsupervised learning does help understand, and predict unknown data better.
